...
class NotesListViewTests(TestCase):
    ...
    def test_list_view_with_search_works_correctly(self):
        ...
        """
        user = create_user(username='TestUser', password='test000')
        self.client.post(reverse('login'), {'username': 'TestUser', 'password': 'test000'})
        create_note(title='TestNote1', user=user)
        create_note(title='TestNote2', user=user)
        response = self.client.get('/notes/?q=TestNote1')  # Did not use reverse. This test will fail if url changes
        self.assertContains(response, 'TestNote1')
...
class NotesDeleteViewTests(TestCase):
    ...
    def test_correct_user_can_delete(self):
        ...
        note = create_note(title='TestNote1', user=user1)  # User1 created the note and they will try to delete it
    ...
    def test_incorrect_user_cannot_delete(self):
        ...
        self.client.login(username='TestUser1', password='test000')
        note = create_note(title='TestNote1', user=user2)  # User2 created the note and user1 will try to delete it
        self.client.post(reverse("notes:delete", kwargs={'slug': note.slug}))
        self.assertEquals(Notes.objects.all().count(), 1)
...
class NotesDetailsViewTests(TestCase):
    ...
    def test_correct_user_can_view(self):
        ...
        user1 = create_user(username='TestUser1', password='test000')
        self.client.login(username='TestUser1', password='test000')
        note = create_note(title='TestNote1', user=user1)  # User1 created the note and they will try to see it
    ...
    def test_incorrect_user_cannot_view(self):
        ...
        user2 = create_user(username='TestUser2', password='test000')
        self.client.login(username='TestUser1', password='test000')
        note = create_note(title='TestNote1', user=user2)  # User2 created the note and user1 will try to view it
        response = self.client.get(reverse("notes:detail", kwargs={'slug': note.slug}))
...
class NotesUpdateViewTests(TestCase):
    ...
    def test_correct_user_can_update(self):
        ...
        user1 = create_user(username='TestUser1', password='test000')
        self.client.login(username='TestUser1', password='test000')
        note = create_note(title='TestNote1', user=user1)  # User1 created the note and they will try to update it
        self.client.post(reverse(
            "notes:update",
            kwargs={'slug': note.slug}), {'title': 'Updated_test_note', 'content': 'test_content', 'category': 'CS'})
    ...
    def test_incorrect_user_cannot_update(self):
        ...
        user2 = create_user(username='TestUser2', password='test000')
        self.client.login(username='TestUser1', password='test000')
        note = create_note(title='TestNote1', user=user2)  # User2 created the note and user1 will try to update it
        self.client.post(reverse(
            "notes:update",
            kwargs={'slug': note.slug}), {'title': 'Updated_test_note', 'content': 'test_content', 'category': 'CS'})
